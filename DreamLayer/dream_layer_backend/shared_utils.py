...
def wait_for_image(prompt_id: str, save_node_id: str = "9", max_wait_time: int = 120) -> List[Dict[str, Any]]:
    """
                            ...
                            if save_node_id in outputs:
                                ...
                                if images_data:
                                    ...
                                    for img_info in images_data:
                                        ...
                                        if filename:
                                            # Try to copy to served images directory, but don't fail if it doesn't work
                                            ...
                                            if os.path.exists(src_path):
                                                try:
                                                    ...
                                                    print(f"⚠️ Warning: Could not copy {filename} to served directory: {copy_error}")
                                                    print(f"⚠️ Will serve directly from output directory")
                                            
                                            # Create proper image object with URL (serve from output directory if needed)
    ...
    # Check if any images were generated even if we timed out
    try:
        output_dir, _ = get_directories()
        # Look for any DreamLayer images in the output directory
        import glob
        dreamlayer_images = glob.glob(os.path.join(output_dir, "DreamLayer_*.png"))
        if dreamlayer_images:
            # Sort by modification time to get the latest
            dreamlayer_images.sort(key=os.path.getmtime, reverse=True)
            latest_images = dreamlayer_images[:4]  # Get the 4 most recent
            
            image_objects = []
            for img_path in latest_images:
                filename = os.path.basename(img_path)
                print(f"📸 Found timeout image: {filename}")
                
                # Try to copy to served images directory
                dest_path = os.path.join(SERVED_IMAGES_DIR, filename)
                try:
                    shutil.copy2(img_path, dest_path)
                    print(f"✅ Copied timeout image to served directory: {filename}")
                except Exception as copy_error:
                    print(f"⚠️ Could not copy timeout image: {copy_error}")
            ...
            if image_objects:
                print(f"🎉 Returning {len(image_objects)} timeout images")
                return image_objects
    except Exception as e:
        print(f"Error checking for timeout images: {e}")
...
def serve_image(filename: str) -> Any:
    ...
    try:
        ...
        if os.path.exists(served_filepath):
            print(f"✅ Serving image from served_images: {filename}")
        ...
        if os.path.exists(input_filepath):
            print(f"✅ Serving image from input directory: {filename}")
        ...
        if os.path.exists(output_filepath):
            print(f"✅ Serving image from output directory: {filename}")

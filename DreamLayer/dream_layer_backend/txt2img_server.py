...
def handle_txt2img():
    ...
    try:
        ...
        # Check if any images were generated despite the error
        try:
            output_dir, _ = get_directories()
            import glob
            ...
            if dreamlayer_images:
                ...
                for img_path in latest_images:
                    ...
                    print(f"📸 Found error recovery image: {filename}")
                    
                    # Try to copy to served images directory
                    dest_path = os.path.join(SERVED_IMAGES_DIR, filename)
                    try:
                        shutil.copy2(img_path, dest_path)
                        print(f"✅ Copied error recovery image to served directory: {filename}")
                    except Exception as copy_error:
                        print(f"⚠️ Could not copy error recovery image: {copy_error}")
                ...
                if image_objects:
                    print(f"🎉 Returning {len(image_objects)} error recovery images")
                    return jsonify({
                        "status": "success",
                        "message": f"Generated {len(image_objects)} images (recovered from error)",
                        "comfy_response": {"all_images": image_objects},
                        "generated_images": image_objects
                    })
